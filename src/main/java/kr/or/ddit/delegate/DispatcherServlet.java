...
public class DispatcherServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// request객체의 getRequestDispatcher 메소드를 호출하여 requestDispatcher 객체를 확보
		//												(어떤 jsp, servler에게 위임할지 인자를 전달)
		// requestDispatcher객체의 forward메소드를 통해 다른 jsp(servlet)에게 응답 생성을 위임
		// 위임 받은 jsp, servlet에서 최종적으로 응답을 생성
		
		// 서블릿에서 요청을 받고 로직을 처리하며 화면에서 필요로 하는 데이터를 request 객체의 속성으로 저장
